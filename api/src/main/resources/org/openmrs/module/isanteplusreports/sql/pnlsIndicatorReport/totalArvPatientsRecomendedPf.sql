...
FROM isanteplus.patient p, isanteplus.patient_on_arv pa,
  ...
  MAX(plf.encounter_date) AS visit_date FROM isanteplus.family_planning plf
  WHERE plf.encounter_date BETWEEN :startDate AND :endDate
      GROUP BY 1) B
     ...
     AND fp.encounter_date BETWEEN :startDate AND :endDate
     AND p.voided <> 1
     AND fp.voided <> 1;